      ...
      try {
        if (orderFormContext && orderFormContext.orderForm && orderFormContext.orderForm.items && !hasExecuted) {
          ...
          await Promise.all(itemIds.map(async itemId => {
            try {
              const skuResponse = await axios.get(`/api/catalog_system/pvt/sku/stockkeepingunitbyid/${itemId}`);
              ...
              if (ProductClustersIds !== null) {
                ...
                if (ProductClustersIds.includes('159') || ProductClustersIds.includes('160') || ProductClustersIds.includes('161') || ProductClustersIds.includes('162')) {
                  ...
                  if (ProductClustersIds.includes('159')) {
                    percentualTaxPromises.push(axios.get('/api/rnb/pvt/calculatorconfiguration/3d7d788a-e88e-420a-94b4-b05414a42919'));
                  }

                  if (ProductClustersIds.includes('160')) {
                    percentualTaxPromises.push(axios.get('/api/rnb/pvt/calculatorconfiguration/56e3f05c-725a-4105-868e-20b559ca191a'));
                  }

                  if (ProductClustersIds.includes('161')) {
                    percentualTaxPromises.push(axios.get('/api/rnb/pvt/calculatorconfiguration/2dc9ff66-4920-4c61-b5e9-01d083b7bae7'));
                  }

                  if (ProductClustersIds.includes('162')) {
                    percentualTaxPromises.push(axios.get('/api/rnb/pvt/calculatorconfiguration/8e0dbbfa-e440-4260-bc4e-7d5fe2291a8f'));
                  }
